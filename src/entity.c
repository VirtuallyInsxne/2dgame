...
EntityState gf2d_entity_state_from_name(const char *state)
{
    ...
    if (strcmp(state,"jumping")==0)return ES_Jumping;
    if (strcmp(state,"dashing")==0)return ES_Dashing;
...
Uint32 gf2d_entity_set_action(Entity *self, const char *action)
{
    ...
    return gf2d_action_get_frames_remaining(self->figureInstance.action,self->frame);
}
...
void gf2d_entity_update(Entity *self)
{
    ...
    self->figureInstance.nextFrame = gf2d_action_next_frame_after(self->figureInstance.action,self->frame);
